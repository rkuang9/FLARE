...
public:
    ...
    Tensor<OutputRank> operator()(const Tensor<TensorSampleRank> &input);
...
template<int OutputRank, int TensorSampleRank>
Tensor<OutputRank> Sequential::operator()(const Tensor<TensorSampleRank> &input)
{
    return this->template Predict<>(input);
}
